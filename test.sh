...
command_log="$(mktemp "${TMPDIR:-/tmp}/mcpa-ising2d-command.XXXXXX")"
...
case "${mode}" in
    cpu) targets=(mcpa_ising2d_cpu_tests) ;;
    gpu) targets=(mcpa_ising2d_cuda_tests mcpa_ising2d_process_tests main_2d_ising) ;;
    checkpoint) targets=(mcpa_ising2d_cpu_tests mcpa_ising2d_cuda_tests mcpa_ising2d_process_tests main_2d_ising) ;;
    all|list) targets=(mcpa_ising2d_cpu_tests mcpa_ising2d_cuda_tests mcpa_ising2d_process_tests main_2d_ising) ;;
esac
...
run_ctest_compact() {
    ...
    list_log="$(mktemp "${TMPDIR:-/tmp}/mcpa-ising2d-ctest-list.XXXXXX")"
    test_log="$(mktemp "${TMPDIR:-/tmp}/mcpa-ising2d-ctest-case.XXXXXX")"
...
case "${mode}" in
    ...
    checkpoint)
        run_ctest_compact --test-dir "${build_dir}" -L checkpoint \
            --output-on-failure --timeout "${MCPA_TEST_TIMEOUT:-60}"
        ;;
    all)
